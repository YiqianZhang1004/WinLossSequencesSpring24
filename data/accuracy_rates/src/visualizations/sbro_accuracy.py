...
for season in range(2007, 2023):
    ...
    elo_rate = accuracy_rates.get_accuracy("sbro", "elo", [season], -1, -1, -1, -1, "", "")
    moneyline_rate = accuracy_rates.get_accuracy("sbro", "moneyline", [season],-1,-1,-1,-1, "", "")
    poll_rate = accuracy_rates.get_accuracy("sbro", "poll", [season], -1, -1, -1, -1, "", "")
...
plt.xlabel('Season')
plt.ylabel('Accuracy')
plt.title('SBRO Accuracy of Methods Over the Years')
plt.legend()
plt.legend(handles=[elo_line, moneyline_line, poll_line])
plt.grid(True)

plt.savefig(r'data\accuracy_rates\visualizations\sbro_accuracy.png')
plt.show()
